...
class SVDRecommender(BaseRecommender):
    ...
    def _sgd(self,
        ...
        pu = np.random.normal(
            ...
            size=(len(self.user_id2idx), self.n_factors)
        )
        qi = np.random.normal(
            loc=self.init_mean,
            scale=self.init_std_dev,
            size=(len(self.item_idx2id), self.n_factors)
        )
    ...
    def estimate(self,
        ...
        if self.biased:
            ...
            estimate += self.bu[u_idx]
            estimate += self.bi[i_idx]
            estimate += np.dot(self.qi[i_idx], self.pu[u_idx])
        else:
            estimate = np.dot(self.qi[i_idx], self.pu[u_idx])
        return estimate


    def recommend_top_n_item(self,
                             user_id: int,
                             topn: int=10) -> tuple:
        u_idx = self.user_id2idx[user_id]
        if self.biased:
            estimates = np.dot(self.qi, (self.pu[u_idx] + self.bu[u_idx]).T) + self.global_mean
        else:
            estimates = np.dot(self.qi, self.pu[u_idx].T)

        indices = estimates.argsort()[::-1]
        indices = indices[:topn]
        item_ids = np.vectorize(lambda x: self.item_idx2id[x])(indices)

        return (item_ids, estimates[indices])
